...
"""
This version of the IIRABM DRL environment uses the updated version of the IIRABM (rule matrix version) and will act for 12 hours, then regress a predicted oxygen deficit instead of using the true oxydef.
...
def createIIRABM(OH, IS, NRI, NIR, injNum, seed, numCytokines=9, abxMult=-1.0):
...
class Iirabm_Environment(gym.Env):
    ...
    def reset(self, OH=.08, IS=4, NRI=0, NIR=2, injNum=27, seed=0, numCytokines=9, abxMult=-1.0, display=False):
        if display:
        ...
        self.ptrToEnv = createIIRABM(OH, IS, NRI, NIR, injNum, seed, numCytokines, abxMult)  # create a new iirabm environment
